...
public partial class FinalDrivingAttack : AttackPattern
{
	...
	private float[] laneXPositions = new float[5];
	...
	)
	{
		...
		await box.SetMode(BattleBoxMode.CarFiveLanes);
		player.Spawn();
	...
	private void SetupLanes()
	{
		...
		laneXPositions[0] = min.X + boxWidth * (1f / 10f);
		laneXPositions[1] = min.X + boxWidth * (3f / 10f);
		laneXPositions[2] = min.X + boxWidth * (5f / 10f);
		laneXPositions[3] = min.X + boxWidth * (7f / 10f);
		laneXPositions[4] = min.X + boxWidth * (9f / 10f);
	...
	private void HandleInput()
	{
		...
		else if (Input.IsActionJustPressed("ui_right") && currentLane < 4)
		{
			currentLane++;
			_ = SnapToLane(currentLane);
		}
	...
	private void HandleDashLineSpawning(double delta)
	{
		...
		float boxWidth  = max.X - min.X;
		float divider1X = min.X + boxWidth * (2f / 10f);
		float divider2X = min.X + boxWidth * (4f / 10f);
		float divider3X = min.X + boxWidth * (6f / 10f);
		float divider4X = min.X + boxWidth * (8f / 10f);

		SpawnDashLine(divider1X, min.Y, max.Y);
		SpawnDashLine(divider2X, min.Y, max.Y);
		SpawnDashLine(divider3X, min.Y, max.Y);
		SpawnDashLine(divider4X, min.Y, max.Y);
	}
