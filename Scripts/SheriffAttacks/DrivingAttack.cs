...
/// </summary>
public partial class DrivingAttack : AttackPattern
{
	...
	// Tracks which scripted events have already fired, keyed by event index
	...
	// -------------------------------------------------------------------------
	// Scripted timeline — add your events here using SpawnAt().
	// Each event needs a unique ID (just increment the number).
	...
	// -------------------------------------------------------------------------
	// Fires a spawn event once when elapsedTime passes its threshold.
	// id must be unique per SpawnAt call in HandleTimeline.
	// -------------------------------------------------------------------------
	...
	private float GetSpeed(PackedScene scene)
	{
		return scene == carScene ? scrollSpeed * carSpeedMultiplier : scrollSpeed;
	}
